...
\quad\text{(four-quadrant; Ada: }\mathtt{Arctan(Y\Rightarrow s_a, X\Rightarrow c_a)}\text{)}
\]
...
Constant direction \(\Rightarrow \varepsilon=0\Rightarrow\sigma=0\). Oscillating /
nearly uniform directions push \(\varepsilon\to 1\) and
\(\sigma\to\pi/\sqrt{3}\).
